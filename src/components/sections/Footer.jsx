...
const Footer = () => {
  ...
  useEffect(() => {
    const initializeChat = async () => {
      ...
      const formattedMessages = existingMessages.map(msg => chatService.formatMessage(msg));
  ...
  return (
    <footer className="bg-[#1A1A1A] relative border-t-2 border-gray-600 py-6">
      {/* Grid pattern subtle */}
      <div 
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6">
        ...
        <div className="flex flex-col lg:flex-row gap-6">
          ...
          <div className="lg:w-2/5">
            <div className="grid gap-4">
              {/* Crafting Table */}
              <div className="bg-black/70 border-2 border-gray-600 p-4" style={{ 
                boxShadow: 'inset -2px -2px 0 0 #000, inset 2px 2px 0 0 #666',
                imageRendering: 'pixelated'
              }}>
                <div className="flex items-center gap-3 mb-3">
                  <div className="w-5 h-5 bg-brown-600 border border-gray-700" style={{ imageRendering: 'pixelated' }}></div>
                  <h3 className="text-white font-bold text-base">CRAFTING TABLE</h3>
                </div>
                <div className="grid grid-cols-1 gap-2 text-sm">
                  <div className="flex items-center gap-2 text-blue-400 hover:text-blue-300 transition-colors">
                    <div className="w-3 h-3 bg-blue-500 border border-blue-700" style={{ imageRendering: 'pixelated' }}></div>
                    <span>React & Vite</span>
                  </div>
                  <div className="flex items-center gap-2 text-cyan-400 hover:text-cyan-300 transition-colors">
                    <div className="w-3 h-3 bg-cyan-500 border border-cyan-700" style={{ imageRendering: 'pixelated' }}></div>
                    <span>Tailwind CSS</span>
                  </div>
                  <div className="flex items-center gap-2 text-purple-400 hover:text-purple-300 transition-colors">
                    <div className="w-3 h-3 bg-purple-500 border border-purple-700" style={{ imageRendering: 'pixelated' }}></div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="bg-black/70 border-2 border-gray-600 p-4" style={{ 
                  ...
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    />
                    <h3 className="text-white font-bold text-sm">PLAYER INFO</h3>
                  </div>
                  <div className="space-y-2 text-sm">
                    <div>
                      <div className="text-gray-400 text-xs">Nome do Jogador</div>
                      <div className="text-white font-bold">Caroline Castro</div>
                    </div>
                    <div>
                      <div className="text-gray-400 text-xs">Especialização</div>
                      <div className="text-yellow-400 font-bold">Frontend Developer</div>
                    </div>
                    <div>
                ...
                <div className="bg-black/70 border-2 border-gray-600 p-4" style={{ 
                  ...
                  <div className="flex items-center gap-2 mb-3">
                    <div className="w-5 h-5 bg-green-600 border border-green-700" style={{ imageRendering: 'pixelated' }}></div>
                    <h3 className="text-white font-bold text-sm">MULTIPLAYER</h3>
                  </div>
                  <div className="space-y-2 text-sm">
                    <a 
                    ...
                    >
                      <div className="w-3 h-3 bg-blue-600 border border-blue-800 group-hover:bg-blue-500" style={{ imageRendering: 'pixelated' }}></div>
                    ...
                    >
                      <div className="w-3 h-3 bg-purple-600 border border-purple-800 group-hover:bg-purple-500" style={{ imageRendering: 'pixelated' }}></div>
                      <span>Portfolio World</span>
                    </a>
                    <div className="flex items-center gap-2 text-green-400">
                      <div className="w-3 h-3 bg-green-600 border border-green-800" style={{ imageRendering: 'pixelated' }}></div>
          ...
          <div className="lg:w-3/5">
            <div className="bg-black/80 border-2 border-gray-600 font-mono h-full flex flex-col" style={{ 
              boxShadow: 'inset -2px -2px 0 0 #000, inset 2px 2px 0 0 #666',
              imageRendering: 'pixelated',
              ...
              <div className="bg-black/60 border-b border-gray-600 p-3">
                <div className="flex items-center gap-2">
                  <div className={`w-3 h-3 rounded-full ${isLoading ? 'bg-yellow-500 animate-spin' : 'bg-green-500 animate-pulse'}`}></div>
                  <span className="text-green-400 font-bold text-sm">MINECRAFT CHAT</span>
                ...
                {isLoading && (
                  <div className="flex items-center justify-center py-4">
                    <div className="flex items-center gap-2 text-gray-400">
                      ...
                      <span className="text-sm ml-2">Carregando mensagens...</span>
                    </div>
                  </div>
                )}
                
                {!isLoading && chatMessages.map((msg, index) => (
                  <div key={msg.id || `${msg.time}-${index}`} className="flex items-start gap-2 hover:bg-black/20 p-1 rounded transition-colors">
                    <span className="text-gray-500 text-xs flex-shrink-0 mt-0.5">[{msg.time}]</span>
                    {msg.type === "system" && (
                      <span className="text-green-400 text-sm">[SISTEMA] {msg.content}</span>
                    )}
                    {msg.type === "info" && (
                      <span className="text-green-400 text-sm">[INFO] {msg.content}</span>
                    )}
                    {msg.player && (
                      <>
                        <span className={`${msg.color} font-bold text-sm flex-shrink-0`}>&lt;{msg.player}&gt;</span>
              ...
              <div className="border-t-2 border-gray-600 bg-black/60 p-3">
                <form onSubmit={handleSendMessage} className="flex gap-3">
                  <input
                    ...
                    className="w-32 px-3 py-2 bg-black/70 border-2 border-gray-700 text-white text-sm font-mono placeholder-gray-500 focus:outline-none focus:border-gray-500 focus:bg-black/80 transition-all"
                    style={{ imageRendering: 'pixelated' }}
                  />
                  <input
                    type="text"
                    ...
                    className="flex-1 px-3 py-2 bg-black/70 border-2 border-gray-700 text-white text-sm font-mono placeholder-gray-500 focus:outline-none focus:border-gray-500 focus:bg-black/80 transition-all"
                    style={{ imageRendering: 'pixelated' }}
                  />
                  <button
                    type="submit"
                    className="px-4 py-2 bg-green-700 hover:bg-green-600 border-2 border-green-800 hover:border-green-700 text-white text-sm font-bold transition-all transform hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none flex items-center justify-center"
                    style={{ 
                      imageRendering: 'pixelated',
                      paddingTop: '10px',
                      paddingBottom: '6px'
        ...
        <div className="text-center mt-6 pt-4 border-t border-gray-700/50">
          <div className="text-gray-500 text-sm font-mono">
            © {new Date().getFullYear()} Caroline Castro • Minecraft Experience ⛏️
          </div>
        </div>
      </div>
    </footer>
